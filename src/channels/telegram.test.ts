...
import { parseTelegramUpdate, verifyTelegramRequest, telegramConfigured } from './telegram';
...
describe('telegramConfigured', () => {
  it('needs both the token and the webhook secret', () => {
    ...
    expect(telegramConfigured(envWith({ TELEGRAM_WEBHOOK_SECRET: undefined }))).toBe(false);
    expect(telegramConfigured(envWith({ TELEGRAM_BOT_TOKEN: undefined }))).toBe(false);
  });
});
...
describe('verifyTelegramRequest', () => {
  ...
  it('accepts the registered secret', () => {
    expect(verifyTelegramRequest(request('webhook-secret'), envWith())).toBe(true);
  });
