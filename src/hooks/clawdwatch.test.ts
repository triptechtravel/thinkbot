...
import { signPayload, SIGNATURE_HEADER, TIMESTAMP_HEADER, ALERT_SCHEMA_VERSION } from 'clawdwatch';
import type { AlertEvent, CheckSummary } from 'clawdwatch';
import { verifyAlert, alertToPrompt, alertHeadline, inboxConfigured } from './clawdwatch';
...
describe('verifyAlert', () => {
  ...
  it('rejects a signature made with a different secret', async () => {
    const body = JSON.stringify(opened());
    expect(await verifyAlert(await signedRequest(body, 'wrong-secret'), body, env)).toBe(false);
  });
  ...
  it('rejects everything when no secret is configured', async () => {
    const body = JSON.stringify(opened());
    expect(await verifyAlert(await signedRequest(body), body, {} as Env)).toBe(false);
  });
});
