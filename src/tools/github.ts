...
export function githubTools(env: Env) {
  ...
  function owner(): string {
    if (!env.GITHUB_OWNER) throw new NotConfiguredError('GitHub', 'GITHUB_OWNER');
    return env.GITHUB_OWNER;
  }

  function token(): string {
    if (!env.GITHUB_TOKEN) throw new NotConfiguredError('GitHub', 'GITHUB_TOKEN');
  ...
  return {
    recentDeploys: tool({
      ...
      execute: async ({ repo, withinMinutes = 120 }) => {
        ...
        return {
          ...
          merged,
          note: merged.length === 0 ? 'Nothing merged in this window.' : undefined,
        };
      },
    }),
    ...
    recentWorkflowRuns: tool({
      ...
      execute: async ({ repo, withinMinutes = 120, onlyFailures = false }) => {
        ...
        return { repo: `${owner()}/${repo}`, windowMinutes: withinMinutes, runs };
      },
    }),
  };
}
