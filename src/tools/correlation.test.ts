  ...
  it("returns only PRs merged inside the window", async () => {
    fetchMock.mockResolvedValue(
      respond([
        {
          number: 482,
          title: "Change auth middleware",
          merged_at: minsAgo(10),
          user: { login: "isaac" },
          html_url: "u1"
        },
        {
...
describe("datadog.datadogQuery", () => {
      ...
      series: [
        {
          metric: "errors",
          scope: "worker:images",
          pointlist: values.map((v, i) => [
            NOW - (values.length - i) * 60_000,
            v
          ])
        }
      ]
  ...
  it("surfaces a step change as a ratio — the number the decision rests on", async () => {
    // Flat, then 4x.
    fetchMock.mockResolvedValue(
      series([...Array(10).fill(5), ...Array(10).fill(20)])
    );
    const result: any = await datadogTools(env()).datadogQuery.execute!(
      { query: "sum:errors{*}" },
      {} as never
  ...
  it("defaults to issues that FIRST appeared in the window", async () => {
    fetchMock.mockResolvedValue(
      respond([
        ...
        issue({
          id: "2",
          title: "Long-standing noise",
          firstSeen: minsAgo(5000)
        })
      ])
    );
